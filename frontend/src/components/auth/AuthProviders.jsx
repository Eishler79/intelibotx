import React, { useState } from 'react';
import { useAuth } from '../../contexts/AuthContext';

const AuthProviders = ({ onSuccess }) => {
  const [loadingProvider, setLoadingProvider] = useState(null);
  const { login } = useAuth();

  const handleProviderAuth = async (provider) => {
    setLoadingProvider(provider);
    
    try {
      // Por ahora implementamos la funcionalidad base
      // En el futuro aquí irán las integraciones OAuth reales
      
      if (provider === 'binance') {
        // Para Binance OAuth, redirigiríamos a Binance OAuth flow
        console.log('Binance OAuth - To be implemented');
        alert('Binance OAuth integration coming soon!');
      } else if (provider === 'google') {
        // Google OAuth integration
        console.log('Google OAuth - To be implemented');
        alert('Google OAuth integration coming soon!');
      } else if (provider === 'apple') {
        // Apple OAuth integration
        console.log('Apple OAuth - To be implemented');  
        alert('Apple OAuth integration coming soon!');
      } else if (provider === 'facebook') {
        // Facebook OAuth integration
        console.log('Facebook OAuth - To be implemented');
        alert('Facebook OAuth integration coming soon!');
      }
      
    } catch (error) {
      console.error(`${provider} auth error:`, error);
    } finally {
      setLoadingProvider(null);
    }
  };

  const AuthButton = ({ provider, icon, label, bgColor, textColor, hoverBg }) => (
    <button
      onClick={() => handleProviderAuth(provider)}
      disabled={loadingProvider === provider}
      className="w-full p-4 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed mb-3"
      style={{
        background: bgColor,
        color: textColor,
        border: '1px solid #3a3f5a'
      }}
      onMouseEnter={(e) => {
        if (loadingProvider !== provider) {
          e.target.style.background = hoverBg;
          e.target.style.transform = 'translateY(-1px)';
          e.target.style.boxShadow = '0 4px 20px rgba(0, 0, 0, 0.1)';
        }
      }}
      onMouseLeave={(e) => {
        if (loadingProvider !== provider) {
          e.target.style.background = bgColor;
          e.target.style.transform = 'translateY(0)';
          e.target.style.boxShadow = 'none';
        }
      }}
    >
      {loadingProvider === provider ? (
        <>
          <svg className="animate-spin h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          Connecting...
        </>
      ) : (
        <>
          {icon}
          <span>{label}</span>
        </>
      )}
    </button>
  );

  return (
    <div className="space-y-4">
      {/* Binance OAuth - Estilo azul */}
      <AuthButton
        provider="binance"
        bgColor="linear-gradient(135deg, #3b82f6 0%, #60a5fa 100%)"
        textColor="#ffffff"
        hoverBg="linear-gradient(135deg, #2563eb 0%, #3b82f6 100%)"
        label="Binance"
        icon={
          <div className="flex items-center justify-center w-8 h-8 rounded-lg" style={{ background: 'rgba(255, 255, 255, 0.2)' }}>
            <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2L13.09 8.26L22 9L13.09 15.74L12 22L10.91 15.74L2 15L10.91 8.26L12 2Z"/>
            </svg>
          </div>
        }
      />

      {/* OKX - Estilo azul */}
      <AuthButton
        provider="okx"
        bgColor="rgba(30, 58, 138, 0.6)"
        textColor="#ffffff"
        hoverBg="rgba(30, 58, 138, 0.8)"
        label="OKX"
        icon={
          <div className="flex items-center justify-center w-8 h-8">
            <div className="grid grid-cols-2 gap-1">
              <div className="w-2 h-2 bg-white rounded-sm"></div>
              <div className="w-2 h-2 bg-white rounded-sm"></div>
              <div className="w-2 h-2 bg-white rounded-sm"></div>
              <div className="w-2 h-2 bg-white rounded-sm"></div>
            </div>
          </div>
        }
      />

      {/* Social Auth Providers Row */}
      <div className="grid grid-cols-3 gap-3">
        {/* Google */}
        <button
          onClick={() => handleProviderAuth('google')}
          disabled={loadingProvider === 'google'}
          className="p-4 rounded-lg transition-all duration-300 flex items-center justify-center disabled:opacity-50"
          style={{
            background: 'rgba(30, 58, 138, 0.4)',
            border: '1px solid rgba(59, 130, 246, 0.5)'
          }}
          onMouseEnter={(e) => {
            if (loadingProvider !== 'google') {
              e.target.style.background = 'rgba(30, 58, 138, 0.6)';
            }
          }}
          onBlur={(e) => {
            if (loadingProvider !== 'google') {
              e.target.style.background = 'rgba(30, 58, 138, 0.4)';
            }
          }}
        >
          {loadingProvider === 'google' ? (
            <svg className="animate-spin h-6 w-6" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="white" strokeWidth="4"></circle>
              <path className="opacity-75" fill="white" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          ) : (
            <svg className="w-6 h-6" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
          )}
        </button>

        {/* Apple */}
        <button
          onClick={() => handleProviderAuth('apple')}
          disabled={loadingProvider === 'apple'}
          className="p-4 rounded-lg transition-all duration-300 flex items-center justify-center disabled:opacity-50"
          style={{
            background: '#2a2f4a',
            border: '1px solid #3a3f5a'
          }}
          onMouseEnter={(e) => {
            if (loadingProvider !== 'apple') {
              e.target.style.background = '#3a3f5a';
            }
          }}
          onMouseLeave={(e) => {
            if (loadingProvider !== 'apple') {
              e.target.style.background = '#2a2f4a';
            }
          }}
        >
          {loadingProvider === 'apple' ? (
            <svg className="animate-spin h-6 w-6" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="white" strokeWidth="4"></circle>
              <path className="opacity-75" fill="white" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          ) : (
            <svg className="w-6 h-6" style={{ color: '#ffffff' }} fill="currentColor" viewBox="0 0 24 24">
              <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
            </svg>
          )}
        </button>

        {/* Facebook */}
        <button
          onClick={() => handleProviderAuth('facebook')}
          disabled={loadingProvider === 'facebook'}
          className="p-4 rounded-lg transition-all duration-300 flex items-center justify-center disabled:opacity-50"
          style={{
            background: '#2a2f4a',
            border: '1px solid #3a3f5a'
          }}
          onMouseEnter={(e) => {
            if (loadingProvider !== 'facebook') {
              e.target.style.background = '#3a3f5a';
            }
          }}
          onMouseLeave={(e) => {
            if (loadingProvider !== 'facebook') {
              e.target.style.background = '#2a2f4a';
            }
          }}
        >
          {loadingProvider === 'facebook' ? (
            <svg className="animate-spin h-6 w-6" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="white" strokeWidth="4"></circle>
              <path className="opacity-75" fill="white" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          ) : (
            <svg className="w-6 h-6" style={{ color: '#1877F2' }} fill="currentColor" viewBox="0 0 24 24">
              <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
            </svg>
          )}
        </button>
      </div>
    </div>
  );
};

export default AuthProviders;